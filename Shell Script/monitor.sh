...
function jiffies_to_percentage {
	...
	diff_utime=$(echo $(( newutime - oldutime )))
	diff_stime=$(echo $(( newstime - oldstime )))
...
function generate_report {
	...
	number_of_files=$(ls /homes/$USER/Desktop/cs252/lab2-src/reports_dir | wc -l)
	first_file=$(ls /homes/$USER/Desktop/cs252/lab2-src/reports_dir | head -n 1)
	if (( $number_of_files >= $MAXIMUM_REPORTS )); then
		rm /homes/$USER/Desktop/cs252/lab2-src/reports_dir/$first_file
	fi
...
function notify
{
	...
	directory="/homes/$USER/Desktop/cs252/lab2-src/reports_dir/"
	file=$(ls -t1 $directory | head -n 1)
	#temp=$(</homes/$USER/Desktop/cs252/lab2-src/reports_dir/ | tail -n 1)
	#Check if the process has exceeded the thresholds
	if (( $cpu_usage_int >= $CPU_THRESHOLD )); then
		$(/usr/bin/mailx -s "CPU threshold exceeded" $USER < /homes/$USER/Desktop/cs252/lab2-src/reports_dir/$file)
	fi

	if (( $mem_usage_int >= $MEM_THRESHOLD )); then
		$(/usr/bin/mailx -s "Mem threshold exceeded" $USER < /homes/$USER/Desktop/cs252/lab2-src/reports_dir/$file)
	fi
